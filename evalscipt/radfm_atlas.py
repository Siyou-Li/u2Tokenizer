...
def main():
    # Initialize paths
    base_dir = '/rds/projects/l/lez-medical-ml/PengyaoQin/download/AbdomenAtalsData/'  # 请替换为您的数据目录路径
    
    # 读取测试集ID的CSV文件
    import pandas as pd
    test_ids = pd.read_csv('/rds/projects/l/lez-medical-ai/PengyaoQin/Ourmodel/IID_test.csv')  # 包含BDMAP ID的CSV文件
    references = pd.read_csv('/rds/projects/l/lez-medical-ai/PengyaoQin/Ourmodel/AbdomenAtlas3.0.csv')  # 包含对应reference text的CSV文件
    ...
    # 第一阶段：生成caption
    for _, row in tqdm.tqdm(test_ids.iterrows(), desc="Generating captions"):
        try:
            BDMAP_ID = str(row['BDMAP ID'])  # 假设列名为'BDMAP ID'
            image_path = os.path.join(base_dir, BDMAP_ID, 'ct.nii.gz')
            
            # 获取对应的reference text
            reference = references[references['BDMAP ID'] == BDMAP_ID]['narrative report'].iloc[0]  # 假设列名为'reference_text'
            
            if not reference.strip():
                print(f"Empty report for {BDMAP_ID}, skipping...")
                continue
            ...
            print(f"Error processing {BDMAP_ID}: {str(e)}")
            continue
