...
def main():
    # Initialize paths
    base_dir = '/rds/projects/l/lez-medical-ml/PengyaoQin/download/CT-RATE/dataset/valid/'  # 请替换为您的数据目录路径
    
    # 读取包含reference的CSV文件

    references = pd.read_csv('/rds/homes/p/pxq307/Compare/valid_labels.csv')  # 包含对应reference text的CSV文件
    ...
    # 第一阶段：生成caption
    for image_path in tqdm.tqdm(nii_files, desc="Generating captions"):
        try:
            # 获取文件名（不含路径和扩展名）
            file_name = os.path.splitext(os.path.splitext(os.path.basename(image_path))[0])[0]
            
            # 在CSV中查找对应的reference text
            reference_row = references[references['VolumeName'] == file_name+".nii.gz"]
            if reference_row.empty:
                print(f"No reference found for {file_name}, skipping...")
                continue
                
            reference = reference_row['Findings_EN'].iloc[0]
            
            if not reference.strip():
                print(f"Empty report for {file_name}, skipping...")
                continue
