...
class NlfTIUtils:
    ...
    def NlfTI_adaptive_resize(self, input_path, output_path, target_size=[256, 256, 32]):
        """
        ...
        ratio = max([target_size[i] / input_shape[i] for i in range(3)])
        if ratio == 1:
        ...
        shutil.move(
            os.path.join(base_path, os.path.basename(output_path)), output_path
            )
